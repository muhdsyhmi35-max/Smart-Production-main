...
/** Wall ms when current session started (first Start / auto shift open); cleared on Reset / new shift day. */
let sessionWorkingStartMs = null;
/** After Stop: efficiency computed with valid plan + working-time inputs until line runs again or reset. */
let stoppedLineEfficiencyValid = false;
/** One-shot guard so TARGET ACHIEVED only runs finalizeEfficiencyOnStop once per session. */
...
/**
 * At line Stop: (actual/plan) × (working time plan / working time actual) × 98%.
 * Working time actual = net minutes session start → stop (scheduled breaks excluded).
 */
function finalizeEfficiencyOnStop() {
  ...
  const grossSec = Math.floor((stopMs - startMs) / 1000);
  const breakSec = scheduledBreakOverlapSec(startMs, stopMs);
  const netSec = Math.max(grossSec - breakSec, 0);
...
window.onload = async function() {
  ...
  if (isMonitor) {
    ...
    const workingPlanEl = document.getElementById("workingTimePlanMin");
    if (workingPlanEl) workingPlanEl.readOnly = true;
    ...
    const workingPlanOp = document.getElementById("workingTimePlanMin");
    if (workingPlanOp) {
      workingPlanOp.addEventListener("input", () => {
